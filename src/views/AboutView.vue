<template>
    <section class="min-h-screen w-full" ref="aboutSection" id="about-section">
        ...
        <div :class="['flex flex-col gap-y-4 xl:grid xl:grid-cols-1 xl:gap-x-5 xl:gap-y-0 mb-36 text-slate-500 dark:text-slate-300 transition-all motion-reduce:transition-none duration-500 delay-300', visible ? 'translate-y-0 opacity-1 blur-0' : 'translate-y-4 opacity-0 blur-sm']">
            <div class="order-2 xl:order-none">
                ...
                <div>
                    ...
                    <ul class="flow flow-col list-['\1F6A9'] pl-6 sm:grid sm:grid-rows-8 sm:grid-flow-col">
                        <li v-for="tech in content.techStack" class="pl-3">{{ tech }}</li>
                    </ul>
                </div>
            </div>
        </div>
